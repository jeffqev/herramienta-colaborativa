...
function TemaEditarForm({ idAsignatura, temaEditar }) {
  // Formularios de antd
  const { Option } = Select;
  ...
  const { temas, editarTema, buscarTemas } = temaContext;
  ...
  useEffect(() => {
    ...
    form.setFieldsValue({
      tema: {
        nombre: temaEditar?.nombre,
        numero: temaEditar?.numero,
        padre: temaEditar?.padre?._id,
      },
    });
  ...
  const onFinish = (values) => {
    ...
    editarTema(temaEditar._id, tema);
  };
  return (
